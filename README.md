...
GSB Connect is a mobile-first, gamified matchmaking web app. Users sign in with Google, set up profile + unique magic key, send/accept/reject match requests, and reveal Instagram only after mutual acceptance.
...
- Zustand
- Firebase Auth (Google Sign-In) + Firebase Admin SDK
...
1. User signs in with Google via the Firebase client SDK.
...
5. Returning users can use `magicKey` in `/api/auth/magic-login` as an alternative login method.
